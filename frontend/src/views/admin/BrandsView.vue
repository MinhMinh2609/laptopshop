<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-800">🏷️ Quản Lý Hãng</h1>
      <button @click="openModal()" class="btn-primary text-sm">+ Thêm Hãng</button>
    </div>
    ...
    <div class="bg-white rounded-2xl border overflow-hidden">
      ...
      <table v-else class="table-admin">
        <thead><tr><th>Tên hãng</th><th>Slug</th><th>Số SP</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
        <tbody>
          <tr v-if="!brands.length">
            <td colspan="5" class="text-center py-12 text-gray-400">Chưa có hãng nào</td>
          </tr>
          <tr v-for="b in brands" :key="b.id">
            <td class="font-semibold text-gray-800">{{ b.name }}</td>
            <td class="font-mono text-xs text-gray-500">{{ b.slug }}</td>
            <td class="text-sm text-gray-600">{{ b.products_count }} sản phẩm</td>
            <td><span :class="b.is_active ? 'badge-success' : 'badge-danger'">{{ b.is_active ? 'Hiển thị' : 'Ẩn' }}</span></td>
            <td>
              <div class="flex gap-2">
                <button @click="openModal(b)" class="text-xs px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100">Sửa</button>
                <button @click="deleteBrand(b)" class="text-xs px-3 py-1.5 bg-red-50 text-red-500 rounded-lg hover:bg-red-100">Xóa</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
      <div class="bg-white rounded-2xl w-full max-w-md p-6">
        <h3 class="font-bold text-gray-800 text-lg mb-5">{{ editItem ? 'Sửa Hãng' : 'Thêm Hãng Mới' }}</h3>
        <form @submit.prevent="saveBrand" class="space-y-4">
          <div>
            ...
            <input v-model="modalForm.name" class="form-input" :class="{'border-red-400':modalErrors.name}"
              placeholder="VD: Asus, Dell, HP..." required />
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm font-medium text-gray-700">Hiển thị hãng</span>
          </label>
...
async function fetchBrands() {
  loading.value = true
  try { const res = await api.get('/admin/brands'); brands.value = res.data.data }
  catch { brands.value = [] }
...
async function saveBrand() {
  ...
  try {
    if (editItem.value) {
      await api.put(`/admin/brands/${editItem.value.id}`, modalForm.value)
      toast.success('Cập nhật hãng thành công!')
    } else {
      await api.post('/admin/brands', modalForm.value)
...
async function deleteBrand(b) {
  if (b.products_count > 0) { toast.error(`Không thể xóa! Hãng có ${b.products_count} sản phẩm.`); return }
  if (!confirm(`Xóa hãng "${b.name}"?`)) return
  try { await api.delete(`/admin/brands/${b.id}`); toast.success('Đã xóa!'); fetchBrands() }
